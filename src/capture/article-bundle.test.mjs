...
describe("captured article bundle", () => {
  it("creates a valid text article bundle", () => {
    ...
    expect(articleBundleToEvidenceAssets(bundle)).toEqual([]);
  });
  ...
  it("converts image references into extractor evidence assets", () => {
    ...
    expect(evidenceAssets.map((asset) => asset.assetId)).toEqual([
  ...
  it("allows image-dominant bundles when readable text is empty", () => {
    ...
    expect(articleBundleToEvidenceAssets(bundle).map((asset) => asset.assetId))
      .toEqual([expect.stringMatching(/^asset-/)]);
  });
