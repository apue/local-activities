...
5. **Agent-readable observability**：脚本和 API 应把生产数据整理成 audit facts、
   candidate index 和 evidence pack，而不是让 Codex 每次临时写查询。
6. **公开验收面**：operator 和 agent 都需要从公开页面回看最终发布效果，包括已
   结束但仍应可访问的 published events。
...
- `agent:audit`：读取最近 N 天生产和 eval 数据，生成 audit fact packet 和
  candidate index；Codex 再判断异常是否成立。
...
-> agent:audit 生成 feedback-related candidates
-> agent:inspect-finding 生成 false-positive evidence pack
-> Codex 判断是否应导出 private negative case
