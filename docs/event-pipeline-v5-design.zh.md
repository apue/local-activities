...
reset-era evaluation surface 运行，但仍有几个明显问题：
...
V5 应被当作一个有限预算 hobby 项目里的真实 agentic pipeline 实验，而不是
一个追逐复杂框架的研究原型。设计优先级如下：

1. 先保证 pipeline 清晰、可替换、可回放，再追求更复杂的模型能力。
2. 每个节点都应小而明确，输入输出可以独立验证。
3. 模型、prompt、provider 和 repair loop 都是可实验变量，不应写死到业务逻辑里。
4. 前沿 LLM app 方法应通过 harness、eval 和 replay 进入项目，而不是通过盲目
   引入依赖进入项目。
5. 自动化发布是长期目标，但自动化必须建立在可观测、可回滚、可解释的节点结果上。

## AI Editor 发布语义

本项目的目标不是把 LLM 产物全部交给人类逐条审核，而是把系统建设成一个
AI Editor：它应当主动识别、整理并发布可参与的公共活动；人类主要以最终用户
身份反馈“哪里不好”，Codex 或其它 coding agent 再通过 ledger、eval 和代码
修复闭环改进系统。

因此发布策略应遵守以下语义：

- `published` 表示后端 policy 认为这是可行动的公共北京活动，字段足够支持用户
  参加或了解活动。
- AI Editor 的正常终态是 `publish`、`discard`、`merge`、`update`。不适合发布
  的文章不应默认进入人工审核，而应带着机器可读 reason code 被终态丢弃。
- `needs_review` 和 `needs_info` 是系统异常队列，不是默认人工审核池。进入这些
  状态的 draft 必须写入机器可读的 `exception_reason_codes`，并保留 editor
  reason。
- `possible_public_activity` 不等于低质量。若它具备时间、地点、主办方、北京范围、
  无重复、无报名证据缺口，并达到高置信线，应允许自动发布。
- 非公众活动、新闻、领导访问、外地活动、缺少关键字段、低置信或重复，应被明确
  终态处理为 `discard`、`merge` 或 `update`。只有无法由系统安全终止的问题，
  例如契约矛盾、解析器缺陷、工具失败或目前不支持但不能安全丢弃的 schedule，
  才进入 `system_exception`。
- 后续 Codex audit 应优先查找两类问题：可发布活动被异常拦下，以及异常 draft
  没有明确 reason code。

## 技术采用原则

默认实现方式是 TypeScript + Zod contract + 项目内自研 lightweight harness。
除非现有 harness 无法解决明确问题，否则不引入 LangChain、LangGraph 或类似
workflow/agent 框架。

如果未来要引入新的 agent 框架或 workflow 框架，必须同时满足：

- 解决一个当前 lightweight harness 无法合理解决的具体问题。
- 不隐藏 prompt、模型响应、attempts、validator issues、成本和延迟。
- 不破坏节点 contract、replay、eval 和 artifact 结构。
- 可以在不重写业务 pipeline 的情况下替换或移除。
- 不把自由工具调用、直接写库或直接发布引入 LLM 节点。

模型供应商也应通过 provider abstraction 接入。切换阿里云、SiliconFlow、
Moonshot、GLM 或其它 provider 时，pipeline contract 不应变化。

## 预算与成本边界

成本是 V5 的一等输出，不是事后日志。每次 live LLM 调用都应记录：

- provider、model、prompt version、schema version。
- input tokens、output tokens、总 token、估算成本。
- latency、attempt number、retry/repair reason。
- 对应 article、pipeline run、pipeline step 和 data class。

预算规则：

- CI 和默认 replay 不调用 live LLM。
- 本地 eval、smoke、production replay 若调用 live LLM，必须显式开启并提供预算上限。
- Cheap Triage 的 live provider 应先用低成本模型验证召回和误杀，再考虑升级模型。
- Full Extract 和 Editor Pass 只应处理路由后的候选内容，或显式选择的 eval case。
- 单次 run 或月度预算超限时应 fail closed：停止、标记 failed 或进入 review，
  不应静默继续消耗。
- Admin 或 ledger 至少能按 run、文章、节点、模型维度汇总 token 和成本。

本文不预设具体月预算金额；具体数值应在 `/goal` 或运行配置中声明。

## 可观测性最低标准

V5 的可观测性不能只依赖日志。Ledger、artifact 和结构化 step record 才是
调试、验收和后续优化的事实来源。

每个 pipeline node 至少应记录：

- input artifact pointer 和 output artifact pointer。
- input hash 和 output hash。
- node name、node version、contract version。
- decision、reason、confidence 或 score。
- validation issues 和 error type。
- startedAt、finishedAt、latency。
- 是否调用外部服务。
- 如果调用 LLM：provider、model、prompt version、schema version、attempts、
  token usage、cost、repair reason。

所有 `published`、`excluded`、`duplicate`、`failed`、以及异常状态
`needs_review` / `needs_info` 都应能从 admin 或只读 ledger 追溯到具体节点原因。
...
| Publish Policy | 发布策略。根据抽取、校验、编辑结果决定 publish、discard、merge、update 或 system_exception。 |
...
- `needs_review` / `needs_info` 只代表系统异常或无法安全终止的例外，不代表默认
  人工审核。
- 每个 published / excluded / duplicate / failed / exception 都写 ledger。
...
- replay 必须能从持久化 artifact 重建节点输入，而不是依赖隐藏日志或内存状态。
...
- live replay 如果重跑模型，应保留新的 raw response；如果只做确定性检查，
  应能读取此前保存的 raw response。
...
- Publish Policy 决策。
- token usage 和模型信息。
- 每个节点的 attempts、latency、cost、prompt/schema version。
- published、excluded、duplicate、failed 和 exception 的具体原因。
- replay 某一步的入口。
...
- 先用项目内 lightweight harness 表达 agentic loop。
- 先通过 provider abstraction 比较模型，再改变 pipeline contract。
- 先用 ledger、artifact 和 eval 证明问题，再引入更重框架。
...
- 能用显式 `allow-live` 和预算参数运行 Cheap Triage 的真实低成本 provider smoke
  或 local eval。
- mock 节点也要写入成本字段占位、attempts 和 observability metadata。
- CI 和默认 replay 不调用 live LLM。
- 不写 production。
...
- 第一版 production 月预算和单次 replay/eval 预算默认值。
- 第一版 admin 可观测性页面展示到哪个粒度。
- 未来引入外部 agent/workflow 框架的触发条件。
