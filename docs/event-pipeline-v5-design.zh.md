...
- `published` 表示后端 policy 认为这是可行动的公共北京活动，字段足够支持用户
  参加或了解活动。
- `needs_review` 和 `needs_info` 是异常队列，不是默认人工审核池。进入这些状态
  的 draft 必须写入机器可读的 `exception_reason_codes`，并保留 editor reason。
- `possible_public_activity` 不等于低质量。若它具备时间、地点、主办方、北京范围、
  无重复、无报名证据缺口，并达到高置信线，应允许自动发布。
- 非公众活动、新闻、领导访问、外地活动、unsupported schedule、缺少关键字段或
  报名证据缺失，应被明确标记为 exception，而不是只留下模糊状态。
...
所有 `excluded`、`non_event`、`needs_review`、`needs_info`、`failed`、
`duplicate` 和 `published` 都应能从 admin 或只读 ledger 追溯到具体节点原因。
...
- excluded、needs_review、needs_info、failed、duplicate 的具体原因。
- replay 某一步的入口。
